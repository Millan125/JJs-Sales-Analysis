def um():
    ...
    if os.path.getsize("master_sales.xlsx") != 0:
        master_df = pd.read_excel("master_sales.xlsx")
    else:
    ...
    while path_dict:
        ...
        new_master_df = pd.concat([master_df, new_df]).sort_index(inplace=False)
        print(path)
        # print(new_df.head())
    ...
    new_master_df.to_excel("master_sales.xlsx", index=False)
